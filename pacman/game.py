...
class Game(object):
    ...
    def run( self ):
        ...
        for i in range(len(self.agents)):
            ...
            if ("registerInitialState" in dir(agent)):
                ...
                if self.catchExceptions:
                    ...
                    '''NEW: in case it is QLearningAgent'''
                    if 'readQtable' in dir(agent):
                        agent.readQtable()
        ...
        while not self.gameOver:
            ...
            '''NEW for QLearningAgent'''
            if 'observeTransition' in dir(agent):
                    
                    agent.doAction(observation, action)
                    nextState = self.state.deepCopy()


                    #REWARD FUNCTION
                    reward = nextState.getScore() - observation.getScore() + 1

                    #llamada al metodo update del agente de QLearning 
                    agent.observeTransition(observation, action, nextState, reward)

                    #increase discount (gamma)
